...
def process_sprite_group(canvas,myset):
    list1 = list(myset)
    for n in range(len(list1)):
        list1[n].draw(canvas)
        if list1[n].update(): 
            myset.remove(list1[n])
    pass        
...
class Sprite:
    ...
    def collide(self, other_object):
        if dist(self.pos, other_object.pos) <= (self.radius + other_object.radius):
            temp_pos = [self.pos[0], self.pos[1]]  
...
def draw(canvas):
    ...
    if lives == 0:
        started = False
        soundtrack.rewind()
        for n in range(len(rock_group)):
            rock_group.pop()
        for n in range(len(explosion_group)):
            explosion_group.pop()
...
def rock_spawner():
    ...
    while dist(my_ship.pos, rock_pos) < 200: 
        rock_pos = [random.randrange(0, WIDTH), random.randrange(0, HEIGHT)]
